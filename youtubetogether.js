  ...
  args =
    args == null
      ...
      : args
          ...
          .split(/ +/g);
  var command =
    args != "" && message.content.charAt(0) === prefix ? args.shift() : false;
  if (command === `w2g`) {
    if (!message.member.voice.channel)
      return message.channel.send(
        "To use this command, you must join a voice channel."
      );
    fetch(
      `https://discord.com/api/v8/channels/${message.member.voice.channelID}/invites`,
      {
        method: "POST",
        body: JSON.stringify({
          max_age: 86400,
